...
def get_foura_cookie(ID=1):
    ...
    :param ID: 数据库中cookie记录的ID，默认为1。
    :return: 解析后的cookie字典。
    """
    engine = sql_orm().get_engine()
    df = pd.read_sql_table('foura', engine)
    df = df[df['ID'] == ID]
    cookies = {}
    for cookie in df.loc[0, 'Cookie'].split(';'):
...
class station():
    def __init__(self):
        ...
        self.folder_temp=f'spider/down/{self.down_name_en}/temp/'
        self.output_path = f"spider/down/{self.down_name_en}/{self.down_name}.xlsx"
    def down(self):
        down_list=['0099977','0099978','0099979','0099980','0099981','0099982','0099983','0099984','0099985','0099986','0099987','0099988','0099989','0099990']
        clear_folder(self.folder_temp)
        for city in down_list:
            for key in ['1','2','4','5']:
                self.data[key]['queryForm:unitHidden']=city
            path = f"{self.folder_temp}{city}{self.down_suffix}"
    ...
    def save_as_sql_xlsx_csv(self,df_list):
        use_cols = {'所属省': 'province', '所属市': 'city', '区县（行政区划）': 'area', '所属区': 'area2', "乡镇（街道）": "village",
                    '行政村（居委会）': 'countryside', '名称': 'site_name',
                    '站址编码': 'site_code', '运维ID': 'site_maitan_code', '站址状态': 'site_status', 'FSU工程状态': 'fsu_status',
    ...
    def main(self):
        ...
        if len(os.listdir(self.folder_temp)) >= 14:
            df_list=self.read_file()# 因为合并所有表再录入数据库会爆内存，所以先读取后遍历录入
...
class station_DC():
    def __init__(self):
        ...
        self.folder_temp = f'spider/down/{self.down_name_en}/temp/'
        self.output_folder=f"spider/down/{self.down_name_en}"
        self.output_path = f"{self.output_folder}/{self.now.strftime('%Y%m')}{self.down_name}.xlsx"
    def down(self):
        clear_folder(self.folder_temp)
        for index, city in enumerate(['0099977,0099978,0099979','0099980,0099981,0099982',
            ...
            path = f"{self.folder_temp}{index}{self.down_suffix}"
            down_file(self.URL, self.data, path)
    ...
    def down(self,guidang):
        ...
        for year in range(start_year, end_year + 1):
            ...
            for month in range(1, max_month + 1):  # 月份循环
                ...
                # 修复缩进：每个月生成独立的文件名并下载
                path = f"{self.folder_temp}{guidang}_{year}_{month}{self.down_suffix}"
                down_file(self.URL, self.data, path)
...
class yunyingshang_order_history():
    def __init__(self,end=datetime.datetime.now()):
        ...
        self.folder_temp=f'spider/down/{self.down_name_en}/temp/'
        self.output_path = f"spider/down/{self.down_name_en}/{self.down_name}{self.begin.strftime('%Y-%m')}.xlsx"

    def down(self):
        clear_folder(self.folder_temp)
        for index,city in enumerate(['0099977','0099978','0099979','0099980','0099981','0099982','0099983','0099984','0099985','0099986','0099987','0099988','0099989','0108648','0108649','0108650','0108651']):
            for belong in ['移动运营商接口','联通运营商接口','电信运营商接口']:
                ...
                path = f"{self.folder_temp}{city}{belong}{self.down_suffix}"
                down_file(self.URL, self.data, path)
    def main(self):
        self.down()
        concat_df(self.folder_temp, self.output_path)
...
class luruyichang():
    ...
    def down(self):
        clear_folder(self.folder_temp)
        for index,city in enumerate(['0099977','0099978','0099979','0099980','0099981','0099982','0099983','0099984','0099985','0099986','0099987','0099988','0099989','0099990']):
            for key in ['1']:
                self.data[key]['queryForm:unitHidden']=city
            path = f"{self.folder_temp}{index}{self.down_suffix}"
            down_file(self.URL,self.data,path)
...
class alarm_history_Hbase():
    ...
    def down(self):
        clear_folder(self.folder_temp)
        for alarm_name in ['一级低压脱离告警','二级低压脱离告警','FSU离线','温度超高','温度过高','交流输入停电告警','总电压过低','直流输出电压过高告警','直流输出电压过低告警'
            ,'门','电池供电告警']:
            ...
            while start<end:
                ...
                for key in ['1']:
                    ...
                    path = f"{self.folder_temp}{alarm_name}{start.strftime('%Y%m%d')}{self.down_suffix}"
                    down_file(self.URL, self.data, path)
                start=start_next
    def main(self):
        self.down()
...
class performence():
    ...
    def main(self,cities,search_id,folder_temp,out_put,hours=24,content_len=102400,cookie_user=1,csv=False):
        def down_by_city(city):
            ...
            down_file(self.URL, self.data, path,xlsx_juge=True,conten_len_error=int(content_len),cookie_user=int(cookie_user))
        for city in cities:
            down_by_city(city)
        # 用于临时下载没成功的文件
        # for city in cities:
...
class alarm_now():
    def __init__(self):
        self.db_fields = [
            ...
            "所属主要告警ID", "告警上报时的站址维护状态", "首次告警时间至目前经过时间（分）", "离首次告警时间是否超过24小时", "信号量说明",
