...
import datetime
import pandas as pd
from sqlalchemy import and_, text  # text 已经导入
import os
import re
import requests
...
class BatteryLifeCaculate():
        ...
        for file in os.listdir(folder):
            try:
                ...
                df = pd.read_excel(path, dtype=str,
                                   usecols=['故障单编码', '工单状态', '派单时间', '站址名称', '申告工单故障分类', '回复内容'])
                df = df.rename(columns={'站址名称': 'site_name', '故障单编码': 'order_id', '派单时间': 'order_time',
                                        '申告工单故障分类': 'order_alarm_type', '回复内容': 'order_response'})
    ...
    def generate_result_shangdan(self):
        with open(settings.resolve_path(r'message\battery_life\out_put_sql_shangdan.txt'), 'r',
        ...
        outage = pd.read_csv(self.down_path, dtype=str, usecols=['站址运维ID', '告警发生时间', '告警名称']).rename(
            columns={"告警发生时间": "本次停电发生时间"})
        outage = (
