...
@router.get("/station_shouzi_index", response_class=HTMLResponse)
async def station_shouzi_index(request: Request):
    ...
    file_times = {}
    for file in files:
        file_path = settings.resolve_path(os.path.join('message/ID_serch/xls', file))
...
@router.post("/station_shouzi")
async def station_shouzi(file: UploadFile = File(None)):
...
@router.get("/station_shouzi_down")
